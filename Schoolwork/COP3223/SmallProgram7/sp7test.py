...
qinputs1 = ['Wanda Vision\nbunny\nab12c,3?ad.!!xO\nturkey key']
qinputs2 = ['St ar W ar s\ndog\n3425hasdf?%d$tTA;b\nstore too']
qinputs3 = ['I ron He art\nleash\nAt&78*7Podfs^\nsubstring stri']
qinputs4 = ['W a k an d a\napology\nsdjhag$^&sd&d\npizza zzaa']
qinputs5 = ['La ngu ag e\nreferee\nxxoO06Dd@#$dD1Ii;:\njavascript avasc']
...
def result():
    ...
    for test in testcases:
        print()
        f1 = open(cwd + '/samplesolutionsp7testcase' + str(scenario) + '.txt', "r")
        f2 = open('sp7student_output_test_case_' + str(scenario) + '.txt', "r")
        ...
        if len(l1) == len(l2) and functools.reduce(lambda x, y : x and y, map(lambda p, q: p == q, l1, l2), True):
            print("TEST CASE " + str(scenario) + " \033[1;92mPASSED!\033[0m")
            passes = passes + 1
        else: 
            print("TEST CASE " + str(scenario) + " \033[1;31mFAILED!\033[0m")
    ...
    if passes == 5:
        print(r"""                    ____________________________________________________________
                   | Hot Dog!!!!! You did it!!!! You passed all the test cases! |
                   | Make sure to check to see that you followed all directions |
                   | that were specified in the assignment. Otherwise Dr.       |
                   | Steinberg and his TAs will deduct points.                  |
                   |____________________________________________________________|
                     ...
                     \   //    | \_/  |    |  \   /                              
                      ...
                      '-'      |  |
                                '-'""")
        print("")
        print("YESSSSS!!! ALL TEST CASES PASSED AND YOU GOT THE DANCING HOT DOG!")
...
def compilerunfile():
    ...
    for subdir, dirs, files in os.walk(cwd):
        for myfile in files:
            if myfile[:13] == 'smallprogram7' and myfile[-2:] == '.c':
                gcccmd = "gcc -o " + "smallprogram7 " + myfile + " -lm" #compile solution file command and generate executable called smallprogram4
    ...
    for test in testcases:
        ...
        #list comprehension
        inputs = ' '.join([str(item) for item in test])
        f = open('sp7student_output_test_case_' + str(scenario) + '.txt', "w")
        f.write("----------------------------------------------------------------------\n")
...
def setupchecker():
    ...
    for x in range(1, 6):
        if not os.path.exists(cwd + "/samplesolutionsp7testcase" + str(x) + ".txt"):
    ...
    if flag == 0:
        raise FileNotFoundError("You are missing your C source!! Please put in your smallprogram7_lastname_firstname.c file. Script Exiting!")
