...
def result():
    ...
    for test in testcases:
        print()
        f1 = open(cwd + '/samplesolutionsp6testcase' + str(scenario) + '.txt', "r")
        f2 = open('sp6student_output_test_case_' + str(scenario) + '.txt', "r")
...
def compilerunfile():
    ...
    for subdir, dirs, files in os.walk(cwd):
        for myfile in files:
            if myfile[:13] == 'smallprogram6' and myfile[-2:] == '.c':
                gcccmd = "gcc -o " + "smallprogram6 " + myfile + " -lm" #compile solution file command and generate executable called smallprogram4
    ...
    for test in testcases:
        ...
        #list comprehension
        inputs = ' '.join([str(item) for item in test])
        f = open('sp6student_output_test_case_' + str(scenario) + '.txt', "w")
        f.write("----------------------------------------------------------------------\n")
...
def setupchecker():
    ...
    for x in range(1, 6):
        if not os.path.exists(cwd + "/samplesolutionsp6testcase" + str(x) + ".txt"):
