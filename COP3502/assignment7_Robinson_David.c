...
int main()
{
    ...
    int totalMass = 0; // track total mass when inputing shipments
    ...
    for (int i = 0; i < numShipments; i++) {
        // If shipment would be invalid by expiring before
        // start or arriving after end, remove it from array
    ...
    while (upperBound - lowerBound > 1e-5) {
        ...
        // If the crew could survive at the guessed consumption rate,
        // see if the crew can survive at a lower consume rate
        if (canDo(shipments, numShipments, mid, start, end))
            lowerBound = mid;

        // Otherwise, the consumption rate needs to be higher
        else
            upperBound = mid;
    }
...
int update(Heap * hp, int oldTime, int newTime, double consumeRate) {
    ...
    while (!isEmpty(hp) && curTime < newTime) {
        ...
        // If the shipment can be finished before the end of the
        // update, consume it and move to next shipment in heap
        ...
        // Otherwise, consume the part of the shipment that would be eaten from current
        // time to end of update at the guessed consumption rate and complete update
        else {
...
int canDo(Shipment * array, int numShipments, double consumeRate, int start, int end) {
    // Create a heap used as a priority queue for shipments that the crew has
    Heap * pq = createHeap();
