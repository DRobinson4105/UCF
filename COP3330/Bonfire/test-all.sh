...
if [ ! -f Bonfire.java ]; then
	echo ""
	echo " Error: You must place Bonfire.java in this directory before we can"
...
do
	...
	# Attempt to compile.
	javac Bonfire.java TestCase$i.java 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " Bonfire.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" Bonfire.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" Bonfire.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" Bonfire.java -n"
		echo "     grep \"\$(printf '^\t')\" Bonfire.java -n"
		echo ""
		echo "  --------------------------------------------------------------"
