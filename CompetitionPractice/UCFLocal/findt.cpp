...
int validT(vector<pair<double, double> >& coords) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = i + 1; j < 4; j++) {
            for (int k = j + 1; k < 4; k++) {
                if (coords[i].first == coords[j].first == coords[k].first) {
                    top3 = {coords[i], coords[j], coords[k]};
                    notUsedCoord -= (i + j + k);
                    direction = 1;
                    break;
                }
                if (coords[i].second == coords[j].second == coords[k].second) {
                    top3 = {coords[i], coords[j], coords[k]};
                    notUsedCoord -= (i + j + k);
                    direction = 2;
                    break;
    ...
    if (direction == 1 && coords[1].first == coords[notUsedCoord].first &&
        coords[1].second != coords[notUsedCoord].second)
            return 1;

    if (direction == 2 && coords[1].second == coords[notUsedCoord].second &&
...
void permute(vector<pair<double, double> >& coords, vector<pair<double, double> >& perm,
    vector<int>& used, int curr, int* count, int length) {
        if (curr == 4) {
            *count += validT(perm);
            return;
        }
        ...
        for (int i = 0; i < length; i++) {
            ...
            used[i] = 1;
            perm[curr] = coords[i];
            permute(coords, perm, used, curr + 1, count, length);
            used[i] = 0;
        }
}
...
int main() {
    ...
    int count = 0;
    permute(coords, perm, used, 0, &count, p);
    cout << count << endl;
